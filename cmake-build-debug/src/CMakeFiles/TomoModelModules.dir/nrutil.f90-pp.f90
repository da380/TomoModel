# 1 "/Users/ij264/Library/CloudStorage/GoogleDrive-ij264@cam.ac.uk/My Drive/PhD/da380/TomoModel/src/nrutil.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
interface swap
   module procedure swap_i,swap_r,swap_d,swap_rv,swap_dv,swap_c, &
        ...
        masked_swap_dv, masked_swap_dm, masked_swap_ds
end interface    
...
! result(i,j) =first_operandi) (op) second_operand(j).
...
SUBROUTINE split_string(instring, string1, string2, delim)
    CHARACTER(len=64), intent(in) :: instring,delim
    CHARACTER(len=64),INTENT(OUT):: string1,string2
    INTEGER :: index
    character(len=64) :: instring2

    instring2 = TRIM(instring)

    index = SCAN(instring2,delim)
    string1 = instring2(1:index-1)
    string2 = instring2(index+1:)
